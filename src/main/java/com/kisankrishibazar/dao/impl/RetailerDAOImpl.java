...
public class RetailerDAOImpl implements RetailerDAO{
	...
	public Boolean registerNewUser(User user){
		...
		int[] types = new int[] {Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,Types.DOUBLE,Types.DOUBLE,Types.VARCHAR,Types.VARCHAR };
		int row = jdbcTemplate.update(query, new Object[] { user.getUsername(),user.getName(),user.getPassword(),user.getType(),user.getLat(),user.getLongt(),user.getAddress(),user.getPhone()},types);
		if(row > 0){
			return true;
		}else{
			return false;
